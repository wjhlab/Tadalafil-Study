...
###Skip lines 544-554 if loading output rds
#read and cluster
...
ggdf3<-melt(exprtbl_cell, id.var=c("cluster","condition","sample_id"))
...
ggdf3<-ggdf3[ggdf3$cluster %nin% c("UA","Multiplet"),]
write.csv(ggdf3, file='MMIpercell.csv') #export MMI per marker in each cluster per cell

pdf("plot_funcmarkers_vio_Pvals2.pdf",width=6,height=7)
for(i in 1:length(fmlistplot)){
  ggp <- ggplot(ggdf3[ggdf3$variable==fmlistplot[i],], aes(x=condition, y=value, fill=condition))+
    ggtitle(fmlistplot[i])+
    facet_wrap(~cluster, ncol=4, scales='free')+
    geom_violin()+
    ylab("MMI")+
    stat_compare_means(method="wilcox.test",label="p.format",size=1,aes(group = group),comparisons = comps)+
...
##make vector of pvalues to be adjusted 
#for each marker, list p values for both comparisons for every relevant immune cluster (e.g., 8 clusters*2= 16 p vals)
p<-c(2.22e-16,0.53,3.23e-15,0.45,2.1e-05,0.32,2.22e-16,1.8e-05,0.17,6.5e-09,5e-05,0.021,0.0093,0.0013) #IAIE
p<-c(0.0013,0.38,0.32,3.2e-16,0.045,0.082,0.22,0.74,0.3,0.75,0.065,0.071,0.29,0.97,0.0012,0.75) #ICOS
p<-c(0.0001,2.22e-16,0.0056,2.22e-16,0.96,6.3e-07,2.2e-05,2.22e-16,6.8e-06,4.9e-16,2.22e-16,2.22e-16,0.11,1.4e-07,4.2e-09,0.33) #PD1
p<-c(2.22e-16,9.9e-13,0.036,2.22e-16,0.091,0.00033,2.9e-15,1.4e-08,2.22e-16,0.0015,0.58,2.22e-16,0.66,3.6e-14,5.9e-05,0.0076) #Tbet
p<-c(0.00024,2.22e-16,6.4e-09,1.1e-12,0.37,2.22e-16,0.12,2.22e-16,0.71,4.5e-05) #GranzymeB

#perform fdr adjustment
fdr<-p.adjust (p, method="fdr") #IAIE
fdr<-p.adjust (p, method="fdr") #ICOS
fdr<-p.adjust (p, method="fdr") #PD1
fdr<-p.adjust (p, method="fdr") #Tbet
fdr<-p.adjust (p, method="fdr") #GranzymeB
...
df1<-data.frame(marker,p,fdr,comp,cluster) #IAIE
df2<-data.frame(marker,p,fdr,comp,cluster) #ICOS
df3<-data.frame(marker,p,fdr,comp,cluster) #PD1
df4<-data.frame(marker,p,fdr,comp,cluster) #Tbet
df5<-data.frame(marker,p,fdr,comp,cluster) #GranzymeB

finaldf<-bind_rows(df1,df2,df3,df4,df5)

write.csv(finaldf,file="C1_Supp_pvals.csv")
