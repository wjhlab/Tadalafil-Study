...
returnfcs <- function(FDR_cutoff=.05,
  ...
  if(is.null(shape_conditions)){shape_conditions <- c(0:20)[1:length(levels(md$condition))]}#can specify as long as number is same
...
do_umap <- function(fcs,subtype_markers,sample_ids,cell_clustering,metadata,
                    clusterMergeFile='~/Desktop/PTPN22 Project/Analysis/Analysis_CD300A_1/Config/CD300a_merged.xlsx',
...
###Skip lines 535-543 if loading output rds
#read and cluster
...
output[8:10] <- clusterfcs(numclusters=15)
...
#clusterlevels=c(1:15)
clusterlevels=c("Klick_Tc",
                 "TcEx_I",
                 "TcEx_II",
                 "TcEx_III",
                 "TcEx_IV",
...
###Skip lines 580-585 if loading output rds
...
immune_counts<-output$immunecounts

#calculate props of immune cells
propsimm_table <- t(t(counts_table) / unlist(immune_counts)) * 100
propsimm <- as.data.frame.matrix(propsimm_table)

write.csv(propsimm, file='propsimm.csv')

#set up the data frame for proportional plotting as a % immune cells
ggdfimm <- melt(data.frame(cluster = rownames(propsimm),propsimm, check.names = FALSE),
...
ggdfimm$vaccine<-factor(output$meta_data$vaccine[match(ggdfimm$sample_id,output$meta_data$sample_id)],levels=txlevels)
ggdfimm<-ggdfimm[ggdfimm$cluster %nin% c("UA"),]

ggdfimm45<-ggdfimm[ggdfimm$condition %in% c("MLIST","MLIST_TAD"),] #df for comparing group 4v5 

#groups for stats
library(ggpubr)
comps<-list(c("VEH","TAD"),c("MLIST","MLIST_TAD"))

#plot box plots: this abundance plot for immune cells: MLIST VS MLIST_TAD 
boxplot3 <- ggplot(ggdfimm45, aes(x=condition, y=proportion, fill=condition))+
...
pdf("plot_abundanceimm_4v5_box.pdf", width=6, height=1.75);boxplot3;dev.off()

## Stacked bars: immune cells in MLIST vs MLIST_TAD
bp2 <- ggplot(ggdfimm45, aes(x = condition, y = proportion, fill = cluster)) +
...
pdf('plot_abundanceimm_4v5_stackedbar.pdf', width=2.5, height=4); bp2; dev.off()


#plot boxplot for Klick_Tc in vax vs no vax
boxplot3 <- ggplot(ggdfimm, aes(x=vaccine, y=proportion, fill=vaccine))+
  geom_boxplot(outlier.size=0, lwd=0.25, color="black")+
  geom_jitter(width=0, size=0.5)+
  facet_wrap(~cluster,ncol=8,scales="free")+
  ylab("% Immune Cells")+
  stat_compare_means(method="t.test", label="p.format",method.args = list(var.equal = TRUE),label.x.npc="center", size=1.5,aes(group = vaccine))+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size=7, color="black"),
        axis.title.x = element_blank(),
        axis.line.x = element_line(size=0.25, color="black"),
...
pdf("plot_vax_abundanceimm_box.pdf", width=6, height=1.75);boxplot3;dev.off()

###T cell focused heatmap
...
keep <- c("Klick_Tc",
          ...
          "TcEx_IV",
          "Tc_CM",
          "Tc_KLRG1") #only include these clusters
...
markerstoshow<-c("Lag3","GZMB","LSIFKHKL","CD137","KI67","PD1","Ly6C","CD44","CD62L","EOMES","Tbet","CD69","CD25","CTLA4","KLRG1","ICOS")
...
pdf('T_Heatmap_selectmarkers.pdf',width=6, height=4);p; dev.off()
...
exprtbl_cell <- data.frame(fsApply(output$fcs,exprs)[, fmlistplot], sample_id = output$sample_ids, cluster = output$cell_clustering1m)

exprtbl_cell$condition <- factor(output$meta_data$condition[match(exprtbl_cell$sample_id,output$meta_data$sample_id)], levels=grouplevels)
exprtbl_cell$vaccine <- factor(output$meta_data$vaccine[match(exprtbl_cell$sample_id,output$meta_data$sample_id)], levels=txlevels)

ggdf3<-melt(exprtbl_cell, id.var=c("cluster","condition","sample_id","vaccine"))
ggdf3$cluster <- factor(ggdf3$cluster, levels=clusterlevels)
ggdf3$sample_id <- factor(ggdf3$sample_id, levels=samplevels)
ggdf3$condition <- factor(ggdf3$condition, levels=grouplevels)
ggdf3$vaccine <- factor(ggdf3$vaccine, levels=txlevels)
ggdf3<-ggdf3[ggdf3$cluster %nin% c("UA"),]
ggdf3_45<-ggdf3[ggdf3$condition %in% c("MLIST","MLIST_TAD"),]

#violin plots for MLIST vs MLIST_TAD
pdf("plot_funcmarkers_4v5_vio.pdf",width=3.5,height=5.4)
for(i in 1:length(fmlistplot)){
  ggp <- ggplot(ggdf3_45[ggdf3_45$variable==fmlistplot[i],], aes(x=condition, y=value, fill=condition))+
    ggtitle(fmlistplot[i])+
    facet_wrap(~cluster, ncol=3, scales='free')+
    geom_violin()+
    ylab("MMI")+
    #geom_boxplot(fill = NA , outlier.shape = NA)+
    stat_compare_means(method="wilcox.test",label.x.npc="center",label="p.format",size=1.5,aes(group = condition))+
